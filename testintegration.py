...
class Drive:
 ...
 #           state = 2 * GPIO.input(STATE1_PIN) + GPIO.input(STATE0_PIN)
            ...
            # Take orders from the Jetson Nano
                ...
                try:
                    throttle_order = 150
                    throttle_order = int(throttle_order)
                    old_throttle_val2 = throttle_order
                except TimeoutOccurred:
                    throttle_order = old_throttle_val2
                    
                # print(throttle_order)
                steering_order = 0
    ...
    def diff_to_delta(self, throttle_diff):
        if throttle_diff>100:
            return throttle_diff*.8
        elif throttle_diff>50:
            return throttle_diff*.8
        elif throttle_diff>10:
            return throttle_diff*.3
        elif throttle_diff>5:
            return throttle_diff*.1
        return throttle_diff*.01
        # if throttle_diff > 100:
        #      return throttle_diff*.9
        # elif throttle_diff > 50:
        #     return throttle_diff*.9
        # else:
        #     return throttle_diff*.1
